...
function ColorGrid(props) {
    var _a = useState(React.createElement(React.Fragment, null)), grid = _a[0], setGrid = _a[1];
    ...
    useEffect(function () {
        if (props.reloadingWithOptions) {
            ...
            if (!options) {
                ...
                setGrid(React.createElement(GridWrapper, { outerShellOnly: options.outerShellOnly, initialVariance: options.initialVariance, rippleVariance: options.rippleVariance, maxGridSize: options.maxGridSize, rippleSpeed: options.rippleSpeed, ripplePropagation: options.ripplePropagation, initialGrayscale: options.initialGrayscale, rippleTransitionSpeed: options.rippleTransitionSpeed }));
            }
        }
    }, []);
    if (props.reloadingWithOptions) {
        return (React.createElement(React.Fragment, null, grid));
    }
    else {
        return (React.createElement(GridWrapper
